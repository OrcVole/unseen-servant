...
`docs/internal/notes/integration-ideas.md` until each hardens here.
...
`docs/internal/recon/cloudron-fit.md`.
...
connect to onions since there is no real CA/DNS involved; and
...
sitemap.xml, /llms.txt, robots.txt), for a capsule that is dual-homed on
...
correctness on platforms where the bound port is not the one clients
...
(docs/internal/recon/prior-art.md), which is the concrete reason no-SNI tolerance
...
address that does not leak the operator's real IP to visitors. Operator
...
take the Atom. Submission/announcement mechanics: `docs/internal/ROADMAP.md` M6.
...
Design source: `docs/internal/recon/smolnet.md`.
