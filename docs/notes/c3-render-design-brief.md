...
`mod.rs` that states the ADR, the trait/type shape, and what's deliberately
*not* here:
...
`Normal`; state at EOF is meaningless (spec is silent — don't error on
...
a renderer that doesn't implement one MUST render that line as plain text,
...
`docs/recon/protocol.md` mandates a title/date convention — gemtext has
...
2. **Watcher debounce window.** BUILD-PLAN names "debounce" with no
   ...
   whether it's config-exposed, and whether debounce coalesces per-file
...
7. **Preformat alt-text in HTML** — `<pre alt="...">`-equivalent (no real
   ...
   the convention; decide what's reasonable now knowing upstream may
...
Prepared by a research pass over `docs/BUILD-PLAN.md`, `docs/adr/0004-dual-surface.md`,
`docs/recon/protocol.md`, `docs/recon/prior-art.md` §1/§"Recommendation for ADR 0001",
