...
- **I2P**: same shape via an I2P server tunnel to the b32 address;
  ...
  exist and hit SNI edge cases: docs/internal/recon/prior-art.md.)
...
(docs/internal/notes/integration-ideas.md's "Visit stats" section) should get
...
- Defaults: aggregate-only, configurable retention (e.g. 90 d), off
  switch, documented plainly (what is and is not collected). Honest
...
- **An explicit machine-vs-human content negotiation is probably a
  non-goal / anti-pattern** worth naming so it does not get built by
